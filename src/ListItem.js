...
const ListItem = ({id, title, isDone, priority, handleCheck, changePriority, removeTodo}) => {

    //TODO: rewrite with useEffect()
    function handlePriorityChange(e) {
        ...
        if (selectedPriority && Priorities.hasOwnProperty(selectedPriority)) {
            changePriority(id, e.target.dataset['priority']);
        }
    ...
    return (
        <div className={`item ${isDone ? 'done' : ''}`}>
            <div className='item__checkbox' onClick={() => {handleCheck(id, !isDone)}}>
            ...
            <div className='item__remove' onClick={() => {removeTodo(id)}}>
                <svg viewBox="0 0 512 512">
